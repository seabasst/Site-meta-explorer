...
must_haves:
  truths:
    ...
    - "Copy URL buttons have 48px touch targets on mobile"
  artifacts:
...
<task type="auto">
  <name>Task 2: Fix tables, charts, and copy buttons for mobile</name>
  <files>
    src/components/results-table.tsx
    src/components/demographics/country-chart.tsx
    src/components/demographics/age-gender-chart.tsx
  ...
  <action>
1. **results-table.tsx - horizontal scroll:** Both table containers (ad destination URLs table ~L277, sitemap URLs table ~L369) are wrapped in `max-h-[600px] overflow-y-auto` divs. Add `overflow-x-auto` to these same divs (making them `max-h-[600px] overflow-y-auto overflow-x-auto` or just `max-h-[600px] overflow-auto`). Add `min-w-[640px]` to both `<table>` elements so they scroll horizontally on mobile rather than squishing.

2. **results-table.tsx - copy URL button touch targets:** The copy URL buttons (small icon buttons next to URLs, currently ~32px) need padding increased to meet the 48px touch target minimum. Find the copy/clipboard button elements and increase their padding. Change from the current small sizing (e.g., `p-1` or similar) to `p-2.5 -m-1.5` to expand the touch area to ~48px while keeping the visual footprint compact. If the buttons use `h-8 w-8` or similar fixed sizing, change to `min-h-[48px] min-w-[48px]` on mobile using `sm:min-h-0 sm:min-w-0 sm:h-8 sm:w-8` to keep desktop compact but mobile touch-friendly. Alternatively, simply increase padding to `p-3` which gives 12px padding on each side of a 16px icon = 40px, or use `p-3.5` for 48px total.

3. **country-chart.tsx (~L119):** The label column uses `w-36 flex-shrink-0` (144px fixed). Change to `w-20 sm:w-36 flex-shrink-0` so labels are narrower on mobile (80px), leaving more room for the bar chart. The truncation on the country name span already handles overflow.

4. **country-chart.tsx - hover interaction:** The chart uses `onMouseEnter`/`onMouseLeave` for the hoveredCountry state. Add `onClick` handler that toggles the hoveredCountry state (if already hovered, clear it; otherwise set it). This way on touch devices, tapping a country row shows the tooltip/breakdown, and tapping again clears it. The existing onClick for `onSegmentClick` already exists -- combine the hover toggle into the same click: when a country is tapped on mobile, both show the hover details AND trigger the filter. This is already partially done since clicking triggers filter. The hover info is visual enhancement shown via `isHovered` state. Change the `onClick` to ALSO set `setHoveredCountry(country.name === hoveredCountry ? null : country.name)` so tapping toggles the hover detail display.

5. **age-gender-chart.tsx:** Similar hover pattern with `hoveredAge` state and `onMouseEnter`/`onMouseLeave`. Add an `onClick` on each bar group that toggles `setHoveredAge(age === hoveredAge ? null : age)`. The existing `onSegmentClick` is for filtering -- keep both behaviors on click. When user taps an age group bar, it shows the Male/Female/Total breakdown AND applies the visual filter effect.
  </action>
  <verify>
Run `npx next build` to confirm no build errors. At 375px viewport:
- Results tables scroll horizontally
- Copy URL buttons have adequate touch target (~48px)
- Country chart labels are narrower
- Tapping a country row or age group bar shows the breakdown info
  </verify>
  <done>
Tables have horizontal scroll on mobile. Copy URL buttons meet 48px touch target minimum. Country chart labels fit on narrow screens. Chart hover data is accessible via tap on touch devices.
  </done>
</task>
...
- Copy URL buttons have 48px touch targets
...
- Copy URL buttons have minimum 48px touch targets on mobile
