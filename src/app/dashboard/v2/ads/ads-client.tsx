...
// ---------------------------------------------------------------------------
// Client component — accepts server-fetched initial data
// ---------------------------------------------------------------------------

export interface AdsClientProps {
  initialAds: Ad[];
  initialPagination: PaginationData | null;
  initialFilteredStats: FilteredStats | null;
  initialTopBrands: TopBrand[];
  initialCategories: FilterOption[];
  initialFormatOptions: FilterOption[];
  /** brandPageId read from ?brandPageId= on the server. */
  initialBrandFilter: string;
}

export default function AdsClient(props: AdsClientProps) {
  const {
    initialAds,
    initialPagination,
    initialFilteredStats,
    initialTopBrands,
    initialCategories,
  ...
  // Prefer the server-read value, but allow client nav to override.
  const urlBrandPageId = searchParams.get('brandPageId') ?? initialBrandFilter;

  // Data state — seeded from server props so first paint has real content.
  const [filteredStats, setFilteredStats] = useState<FilteredStats | null>(initialFilteredStats);
  const [loadedAds, setLoadedAds] = useState<Ad[]>(initialAds);
  const [pagination, setPagination] = useState<PaginationData | null>(initialPagination);
  const [topBrands, setTopBrands] = useState<TopBrand[]>(initialTopBrands);
  const [nextPage, setNextPage] = useState(2); // Server already fetched page 1
  const [hasMore, setHasMore] = useState(initialPagination?.hasNext ?? false);
  const [isLoadingMore, setIsLoadingMore] = useState(false);

  // Filter options from API — seeded from server.
  const [categories, setCategories] = useState<FilterOption[]>(initialCategories);
  const [formatOptions, setFormatOptions] = useState<FilterOption[]>(initialFormatOptions);
  ...
  // UI state. `loading` starts FALSE because the server already fetched the
  // initial batch — no skeleton flash on first paint. `setLoading` is kept
  // but currently unused; reserve for future states (e.g. a "refresh feed"
  // button) that want the full skeleton rather than the inline adsLoading.
  ...
  // Initial brands + filter options + first page of ads were fetched on the
  // server (see ../ads/page.tsx) and seeded into state above. No mount XHRs
  // here — first paint shows real data. Subsequent filter changes still
  // refetch via the `fetchAds` effect below.
  //
  // These setters are kept so future features (e.g. "refresh top brands"
  // button) have an entrypoint; they're currently no-ops after mount.
  void setTopBrands;
  void setCategories;
  ...
  useEffect(() => {
    if (initialAds.length === 0 || !session?.user?.id) return;
    let cancelled = false;
    fetch('/api/ad-library/saved/check', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ adIds: initialAds.map((a) => a.id) }),
    })
      .then((r) => (r.ok ? r.json() : null))
      .then((data) => {
        if (cancelled || !data) return;
        setSavedAdIds(new Set(data.savedAdIds || []));
      })
      .catch(() => {
        /* non-critical */
      });
  ...
  // Only want this once for the server-rendered set; filter changes refetch via fetchAds.
